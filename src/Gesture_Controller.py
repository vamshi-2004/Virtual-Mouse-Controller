...
class Controller:
    ...
    def changesystembrightness():
        ...
        try:
            # Get current brightness (assumes single display)
            current_brightness = sbcontrol.get_brightness(display=0)[0]
            
            # Convert to 0-1 scale
            brightness_level = current_brightness / 100.0
            
            # Adjust brightness based on pinch level
            brightness_level += Controller.pinchlv / 50.0
            brightness_level = max(0.0, min(1.0, brightness_level))  # Clamp between 0.0 and 1.0
            
            # Set brightness
            sbcontrol.set_brightness(int(brightness_level * 100), display=0)
    ...
    def scrollVertical():
        """Scrolls on screen vertically based on pinch level."""
        try:
            scroll_direction = 120 if Controller.pinchlv > 0.0 else -120
            pyautogui.scroll(scroll_direction)
        except Exception as e:
            print(f"Error during vertical scroll: {e}")

    def scrollHorizontal():
        """Scrolls on screen horizontally based on pinch level."""
        try:
            # Hold down shift and ctrl for horizontal scrolling
            pyautogui.keyDown('shift')
            pyautogui.keyDown('ctrl')
            
            # Scroll left if pinchlv > 0, otherwise right
            scroll_direction = -120 if Controller.pinchlv > 0.0 else 120
            pyautogui.scroll(scroll_direction)
